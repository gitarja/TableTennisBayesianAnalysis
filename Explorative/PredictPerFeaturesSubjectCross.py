...
def evaluateModel(model, X_test, y_test):
    ...
    print(cm)
    print("-------------------------")
    return mcc, cm, acc, auc_pr, f1, g_mean, y_pred
...
for i, (train_index, test_index) in enumerate(kf.split(all_groups)):
    ...
    mcc, cm, acc, auc_score, f1, g_mean, pred = evaluateModel(model, X_test, y_test)
...
all_preds = np.concatenate(y_pred_list)
np.save("Results\\" + label + "_pred.npy", all_preds)
