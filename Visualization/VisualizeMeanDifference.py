...
# features = ["receiver_im_racket_ball_angle"]
...
for analyzed_features in features:
    ...
    data = trace_post["difference_of_means"].data.flatten()
    ...
    plt.plot(pos, kde(pos), color="#076c96",  linewidth=3.0)
    # Set shading bounds
    ...
    plt.fill_between(shade, kde(shade), alpha=0.1, color="#076c96")
    plt.vlines(x=0, ymin=0, ymax=np.max(kde(pos)), color='#000000', linestyles ="dashed") # plot vertical line
    # ymin, ymax = plt.ylim()
    # plt.fill_between(np.arange(-0.05, 0.05, 0.01), ymin, ymax, color='#636363', alpha=.5)
    ...
    # plt.show()
    plt.savefig(DOUBLE_RESULTS_PATH_TTEST + "MeanDifference\\" + analyzed_features + ".pdf", format='pdf', transparent=True)
    plt.close()
