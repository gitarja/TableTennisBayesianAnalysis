...
if __name__ == '__main__':
    ...
    observation_idx = np.unique(clean_df["observation_seg"].values)
    ...
    observation_v_idx = np.searchsorted(observation_idx, clean_df["observation_seg"].values)
    ...
    coords = {
        ...
        "group_idx": group_idx,
        "segment_idx": observation_idx,
        "axis": range(8),
        "feature": range(4),
    }

    model = BayesianHirModel(coords, X_std, y_f_std, priors_mean, priors_std,  session_v_idx, hitter_v_idx, group_v_idx, observation_v_idx)
    ...
    plt.savefig(DOUBLE_RESULTS_PATH_TTEST + "PPC_regression\\minimize_proactive_reactive.png", format='png')
    plt.close()

    trace_post = az.extract(idata.posterior)
    # print(az.summary(idata))
    # save the model
    with open(DOUBLE_RESULTS_PATH_TTEST + "model_regression\\" + "idata_proactive_reactive.pkl", 'wb') as handle:
        print("write data into: " + "idata_ttest_minimize_uncertainty.pkl")
