...
def trainModel(X, y, search_params=False):
    ...
    if search_params:
        ...
        model = RandomForestClassifier(random_state=0, class_weight="balanced", max_depth=7, max_features=.75,
                                       max_leaf_nodes=15, max_samples=.35, n_estimators=100).fit(X, y)
...
for i, (train_index, test_index) in enumerate(kf.split(all_groups)):
    ...
    train_features = train_reader.getStableUnstableFailureFeatures(group_name="train_subjects",
